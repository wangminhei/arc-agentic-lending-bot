/**
 * deploy-multi-collateral.ts
 * Biên dịch và deploy contract AgenticMultiCollateralPool lên Arc Testnet
 */
...
async function main() {
  console.log("=== COMPILING AGENTIC MULTI-COLLATERAL POOL ===");

  const contractPath = path.resolve("./contracts/AgenticMultiCollateralPool.sol");
  ...
  const input = {
    language: "Solidity",
    sources: {
      "AgenticMultiCollateralPool.sol": {
        content: source,
      },
    },
    settings: {
  ...
  const contractOutput = output.contracts["AgenticMultiCollateralPool.sol"]["AgenticMultiCollateralPool"];
  ...
  fs.writeFileSync(
    path.join(abiDir, "MultiCollateralPoolABI.json"),
    JSON.stringify(abi, null, 2)
  );
  console.log(`✓ Saved ABI to runtime/worker-01/state/MultiCollateralPoolABI.json`);
  ...
  // Load Circle Owner address
  const walletsFile = path.join(abiDir, "wallets.json");
  if (!fs.existsSync(walletsFile)) {
    throw new Error(`wallets.json not found at ${walletsFile}. Please run the bot first to generate it.`);
  }
  const walletsState = JSON.parse(fs.readFileSync(walletsFile, "utf8"));
  const circleOwnerAddress = walletsState.owner.address;
  console.log(`Circle Owner Address: ${circleOwnerAddress}`);
  ...
  const hash = await walletClient.deployContract({
    ...
    args: [USDC_CONTRACT, EURC_CONTRACT, CIRBTC_CONTRACT, circleOwnerAddress],
  });
  ...
  console.log(`\n=========================================`);
  console.log(`✅ AgenticMultiCollateralPool deployed successfully!`);
  ...
  contractsConfig["multi_collateral_pool"] = contractAddress;
