/**
 * deploy-escrow.ts
 * Biên dịch và deploy contract AgenticEscrow lên Arc Testnet
 */
...
async function main() {
  console.log("=== COMPILING AGENTIC ESCROW ===");

  const contractPath = path.resolve("./contracts/AgenticEscrow.sol");
  ...
  const contractOutput = output.contracts["AgenticEscrow.sol"]["AgenticEscrow"];
  ...
  fs.writeFileSync(
    path.join(stateDir, "EscrowABI.json"),
    JSON.stringify(abi, null, 2)
  );
  console.log(`✓ Saved ABI to runtime/worker-01/state/EscrowABI.json`);
  ...
  console.log(`\n=========================================`);
  console.log(`✅ AgenticEscrow deployed successfully!`);
  ...
  contractsConfig["escrow"] = contractAddress;
